import Link from "next/link";
import { ArrowLeft } from "lucide-react";

export function Hero() {
  return (
    <div>
      <div
        className="rise-d flex items-center gap-2 text-[12px]"
        style={{ animationDelay: "40ms" }}
      >
        <Link
          href="/"
          className="inline-flex items-center gap-1 text-ink-3 hover:text-ink transition-colors"
        >
          <ArrowLeft className="h-3.5 w-3.5" />
          <span className="overline text-[11px]">返回 · THE COLLECTION</span>
        </Link>
      </div>

      <header
        className="rise mt-6 grid grid-cols-12 items-end gap-6"
        style={{ animationDelay: "120ms" }}
      >
        <div className="col-span-12 lg:col-span-9">
          <div className="overline mb-3" style={{ color: "#6b8ed6" }}>
            学习 · 工具
          </div>
          <h1 className="serif text-[44px] sm:text-[60px] leading-[0.96] tracking-[-0.025em] text-ink">
            <span className="serif-italic text-ink-2">FWMamba-UNet</span>
            <span className="ml-3">端到端剖析</span>
          </h1>
          <p className="mt-4 max-w-2xl text-[14.5px] leading-relaxed text-ink-2">
            把一张{" "}
            <span className="serif-italic text-ink">ISIC 皮肤病变</span>{" "}
            图像送进 <span className="serif-italic text-ink">FWMamba-UNet</span>——
            沿着 Patch Embed → 四级编码器 → 瓶颈 → 三级解码器的通路，逐层打开
            <strong> 小波频率分支（DWT）</strong> 与
            <strong> 边缘感知 Skip（EAFF）</strong> 的内部，最后落到分割掩膜和
            Dice/IoU 指标。
          </p>
        </div>
        <div className="col-span-12 lg:col-span-3 lg:text-right">
          <div className="hairline mb-3 lg:ml-auto lg:w-24" />
          <div className="overline">Based on</div>
          <div className="serif text-[18px] mt-1 text-ink leading-tight">
            FWMamba <span className="serif-italic">UNet</span>
            <div className="mt-1 text-[11px] text-ink-3 font-normal tracking-normal">
              ISIC 2018 · Dice 0.896
            </div>
          </div>
        </div>
      </header>

      <div className="hairline mt-10" />
    </div>
  );
}
