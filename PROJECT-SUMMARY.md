...
本地优先的论文精读与研究推进工作台（Next.js 全栈）：把一篇论文从无到有的流程切成 **6 步主线**（二字动词链）「检索 → 精读 → 提炼 → 立论 → 撰写 → 制图」，工作流工具挂在主线上，上游输出可一键流转为下游输入，产出回存论文条目，闭合「检索—精读—生成—回存」回环。**本地优先**（IndexedDB 单一真相源，零配置可用），云端（Supabase）与 LLM key 全部可选、缺失时优雅降级。另有独立的**可视化展厅**（`track: "gallery"`：5 个模型 explainer + iGEM 叙事页），是交互式教学演示，不入论文库、不参与一键流转。工具注册表 `lib/tools-registry.ts` 用 `track: "workflow" | "gallery"` 区分两者。
