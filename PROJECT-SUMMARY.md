...
本地优先的论文精读与研究推进工作台（Next.js 全栈）：把科研流程切成 **5 环主线**「查论文 → 读文献 → 生 idea → 做验证 → 论文绘图」，工作流工具挂在主线上，上游输出可一键流转为下游输入，产出回存论文条目，闭合「检索—精读—生成—回存」回环。**本地优先**（IndexedDB 单一真相源，零配置可用），云端（Supabase）与 LLM key 全部可选、缺失时优雅降级。另有独立的**可视化展厅**（`track: "gallery"`：5 个模型 explainer + iGEM 叙事页），是交互式教学演示，不入论文库、不参与一键流转。工具注册表 `lib/tools-registry.ts` 用 `track: "workflow" | "gallery"` 区分两者。
...
```
app/
├── page.tsx                     # 首页（5 环走廊 + 工作流工具网格 + 可视化展厅，注册表驱动）
...
├── tools/<slug>/page.tsx        # 工具页（workflow 工作流工具 + gallery 展厅 explainer/叙事页）
...
6. ~~「可视化表达」环内容偏 showcase~~ **（2026-06 已重定位解决）**：工作流从 7 环收为 5 环；`讲结果`/`可视化表达` 两环移除，5 个模型 explainer + iGEM 改标 `track: "gallery"`，从工作流叙事剥离为独立「可视化展厅」（首页单独分区，诚实标注为教学演示）；citation-graph / research-genealogy 收回为纯「查论文」。详见 `IMPROVEMENT-PLAN.md §5`。
