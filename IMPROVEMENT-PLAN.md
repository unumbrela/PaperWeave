...
## 6. 第四轮（2026-06）：以论文为核心重排主线 + 强检索 + 撰写环

**问题**：首页把 5 环 + 展厅 + 工具网格平铺，核心论文主线不突出（偏「工具集合」）；检索缺查询扩展层与新论文通道；
缺「组织撰写」环；检索结果有简介但无「方向定位」。

| 项 | 动作 |
| --- | --- |
| 5 环更名 | ✅ `查论文/读文献/生 idea/做验证/论文绘图` → **`调研搜索/精读定位/创新点/组织撰写/论文绘图`**（以论文为核心叙事） |
| 散乱下沉 | ✅ 新增 `Track="lab"`；`prompt-chunker`/`skill-maker` 转 lab（命令行/自动化扩展），从主线剥离；新增 `getLabTools()` |
| 检索扩源 | ✅ 新增 **Crossref** 源（免 key，1.5 亿 DOI），并入 `searchPapers` 容错链路；默认源加 crossref |
| 查询扩展 | ✅ 新增 `lib/paper-search/query-expand.ts`（LLM 把研究目标拆 3–5 条多样化子查询）；无 key 自动降级单查询 |
| fan-out + 重排 | ✅ `searchPapersExpanded` 对子查询跨源并发 → 合并去重 → `rerankFanout`（命中子查询数 + 时新度 + 引用） |
| 新论文通道 | ✅ SearchForm「🆕 近一年」一键切到近一年 + 最新优先 |
| 论文定位 | ✅ `quick-summaries` 每篇返回 `{summary, position}`；ResultCard 并排展示「简介 + 定位」 |
| 组织撰写环 | ✅ 新增 `paper-writer`（API + 页面）：大纲 + 逐节要点 + 段落脚手架 + Related Work 分组 + 自查清单；**段落级辅助，不代写正文** |
| 首页重构 | ✅ 「核心论文流程」5 环走廊置顶（每环直达代表工具 + 角色说明）；命令行扩展（The Lab）与可视化展厅下沉；Hero/SEO 文案重写 |
