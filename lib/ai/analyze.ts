...
/** 跑一次分析，返回模型原始输出（由调用方决定如何解析/落库）。keys 支持访客自带。 */
export async function callAnalysis(userPrompt: string, keys?: ResolvedKeys): Promise<string> {
  return chatCompletion(
    ...
    {
      model: (keys?.deepseek ?? process.env.DEEPSEEK_API_KEY) ? 'deepseek-chat' : undefined,
      temperature: 0.2,
      max_tokens: 1000,
    },
    keys,
