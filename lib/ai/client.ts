...
import { ZENMUX_URL, ZENMUX_HEADERS } from './zenmux';
import { DEFAULT_ZENMUX_MODEL } from './models';
...
const PLACEHOLDERS = new Set(['', 'your-openai-key', 'your-deepseek-key', 'your_deepseek_key', 'your-gemini-key', 'your-zenmux-key', 'ci-placeholder']);
...
function effectiveKeys(override?: ResolvedKeys) {
  return {
    ...
    zenmux: override?.zenmux ?? process.env.ZENMUX_API_KEY,
  };
}
...
/** ZenMux（OpenAI 兼容网关）：用访客选中的模型一次性补全。 */
async function callZenMux(messages: Message[], key: string, model: string, temperature = 0.3, max_tokens = 1000) {
  const client = new OpenAI({ apiKey: key, baseURL: ZENMUX_URL, defaultHeaders: ZENMUX_HEADERS });
  const res = await client.chat.completions.create({
    model,
    messages: messages as OpenAI.Chat.ChatCompletionMessageParam[],
    temperature,
...
export async function chatCompletion(
  ...
  // 访客显式选定的 ZenMux 模型 → 作为主供应商（DeepSeek 等仍作兜底）
  if (keys?.zenmuxModel && isReal(eff.zenmux))
    attempts.push({ name: `ZenMux(${keys.zenmuxModel})`, fn: () => callZenMux(messages, eff.zenmux!, keys.zenmuxModel!, temperature, max_tokens) });
  ...
  // 仅配了 ZenMux key 又没选型时，用兜底模型保证可用（不抢内置供应商的默认位）
  if (isReal(eff.zenmux) && !keys?.zenmuxModel)
    attempts.push({ name: `ZenMux(${DEFAULT_ZENMUX_MODEL})`, fn: () => callZenMux(messages, eff.zenmux!, DEFAULT_ZENMUX_MODEL, temperature, max_tokens) });

  if (attempts.length === 0) {
    throw new Error('AI 服务未配置：请在右上角「API Key」填入你自己的 key（本地开发可在 .env.local 设 DEEPSEEK_API_KEY / OPENAI_API_KEY / GOOGLE_API_KEY / ZENMUX_API_KEY）。');
  }
