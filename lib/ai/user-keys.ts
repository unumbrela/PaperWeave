 ...
 * 通过 `x-deepseek-key` / `x-openai-key` / `x-gemini-key` / `x-zenmux-key`
 * 头带给对应路由。填了 ZenMux key 的访客还可选一个具体模型（见 lib/ai/models.ts），
 * 通过 `x-zenmux-model` 头随每次请求带上，服务端据此优先用该模型。
 */
...
export function setUserKeys(keys: UserKeys): void {
  ...
  const trimmed: UserKeys = {
    ...
    zenmux: keys.zenmux?.trim() || undefined,
  };
...
export function hasAnyUserKey(): boolean {
  const k = getUserKeys();
  return !!(k.deepseek || k.openai || k.gemini || k.zenmux);
}

/** 访客选中的 ZenMux 模型 id（未选 / 非法返回空字符串）。 */
export function getSelectedModel(): string {
  if (typeof window === 'undefined') return '';
  const v = localStorage.getItem(MODEL_KEY) || '';
  return isZenMuxModel(v) ? v : '';
}

/** 保存选中的 ZenMux 模型（传空字符串=用内置默认链路）。 */
export function setSelectedModel(model: string): void {
  if (typeof window === 'undefined') return;
  if (model && isZenMuxModel(model)) localStorage.setItem(MODEL_KEY, model);
  else localStorage.removeItem(MODEL_KEY);
}

/** 把已保存的 key（及选中的 ZenMux 模型）转成请求头，附加到 AI 请求上。 */
export function userKeyHeaders(): Record<string, string> {
  ...
  if (k.zenmux) h['x-zenmux-key'] = k.zenmux;
  // 仅在选了模型且确有 ZenMux key 时才带模型头（否则服务端会忽略）
  const model = getSelectedModel();
  if (model && k.zenmux) h['x-zenmux-model'] = model;
  return h;
}
