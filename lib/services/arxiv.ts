...
export async function fetchArxivMetadata(arxivId: string): Promise<ArxivMetadata> {
  ...
  if (entry.author) {
    ...
    authors = authorList.map((author: any) => ({
      name: author.name?.['_'] || author.name || 'Unknown',
      affiliation: author.affiliation?.['_'] || author.affiliation,
    }));
  }
  ...
  if (entry.link) {
    ...
    const pdfLink = links.find((link: any) => link.$?.rel === 'alternate' && link.$?.type === 'application/pdf');
    if (pdfLink) {
      pdfUrl = pdfLink.$?.href || '';
    }
  }
